...
def render():
    ...
    try:
        ...
        frames = {
            "gkg": pd.DataFrame(), "cisa": pd.DataFrame(), "fema": pd.DataFrame(),
    ...
    # Dynamic subtitle (after we have numbers)
    ...
    with m1:
        ...
        st.markdown(
            "<div class='calc-note'>Percentile-weighted composite of tone, volume, CISA, FEMA, VIX, TSA.</div>",
    ...
    with m2:
        ...
        st.markdown(
            "<div class='calc-note'>Latest ^VIX close (yfinance history).</div>",
            unsafe_allow_html=True
        )
    ...
    with m3:
        ...
        st.markdown(
            "<div class='calc-note'>TSA 7-day avg vs 2019 7-day avg.</div>",
            unsafe_allow_html=True
        )
    ...
    with m4:
        ...
        st.markdown(
            "<div class='calc-note'>Count of advisories past 72h.</div>",
            unsafe_allow_html=True
        )
    ...
    with m5:
        ...
        st.markdown(
            "<div class='calc-note'>Sum of daily declarations over 14d.</div>",
            unsafe_allow_html=True
        )
    ...
    # RIGHT — Macro & Activity signals
    with right:
        if not market_hist.empty:
            st.markdown("<div class='chart-card'>", unsafe_allow_html=True)
            _section_title("Macro Pulse")
            st.line_chart(
                market_hist[["S&P 500", "Nasdaq 100"]].dropna(),
                use_container_width=True, height=160
            )
            mom = market_momentum(market_hist)
            st.markdown(
                f"<div class='small'>20-day momentum — S&P 500: {mom.get('S&P 500', 0):+.2f}% · "
                f"Nasdaq 100: {mom.get('Nasdaq 100', 0):+.2f}%</div>",
                unsafe_allow_html=True
            )
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        if not tsa_df.empty:
            ...
            st.line_chart(
                tsa_df[["current_7dma", "baseline_7dma"]]
                .rename(columns={"current_7dma": "Current", "baseline_7dma": "2019 Baseline"}),
                height=160, use_container_width=True
            )
            st.markdown(
                f"<div class='small'>Latest Δ vs 2019: {tsa_df['delta_vs_2019_pct'].iloc[-1]:+.1f}%</div>",
                unsafe_allow_html=True
            )
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.caption(
        "Sources: Google News (RSS), GDELT GKG v2, TSA Passenger Volumes, "
        "CISA Advisories, FEMA OpenFEMA, yfinance indices."
    )
