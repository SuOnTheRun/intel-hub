...
def strategist_playbook(breakdown: dict, market_hist: pd.DataFrame, tsa_df: pd.DataFrame, news_df: pd.DataFrame):
    ...
    All bullets are driven by the live inputs you already compute.
    """
    ...
    if comp:
        ...
        if comp["tone"]["risk"] >= 60:
            out_mkt.append("Tighten brand-safety & context filters on hard-news inventory.")
        if comp["tsa"]["risk"] >= 60:
            out_mkt.append("Shift away from travel-moment creatives; lean into at-home/comparison shopping intents.")
    ...
    if comp.get("cisa",{}).get("risk",0) >= 60:
        out_ins.append("Monitor CISA hourly for vendor/library CVEs impacting fintech/retail apps.")
